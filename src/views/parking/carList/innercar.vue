...
import { queryinnercar } from '@/api/parkingYarn'
  ...
  methods: {
    ...
    async handleInit () {
      ...
      try {
        ...
        if (Array.isArray(parkingLotList) && parkingLotList.length) {
          ...
          this.listQuery.parkId = parkingLotList[0].parkId
          this.getList()
        }
      }catch(err) {
        console.log(err)
    ...
    async getList () {
      ...
      try {
        ...
        const res = ({ data: { data: { total: this.total, records: this.tableData } } } = await queryinnercar(this.listQuery))
